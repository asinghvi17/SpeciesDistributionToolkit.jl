...
Base.@kwdef mutable struct Node <: OpenAPI.APIModel
    ...
    function Node(build, _links, created, uuid, names, )
        OpenAPI.validate_property(Node, Symbol("build"), build)
        OpenAPI.validate_property(Node, Symbol("_links"), _links)
        OpenAPI.validate_property(Node, Symbol("created"), created)
        OpenAPI.validate_property(Node, Symbol("uuid"), uuid)
        OpenAPI.validate_property(Node, Symbol("names"), names)
        return new(build, _links, created, uuid, names, )
    end
end # type Node

const _property_types_Node = Dict{Symbol,String}(Symbol("build")=>"Int64", Symbol("_links")=>"NodeAllOfLinks", Symbol("created")=>"ZonedDateTime", Symbol("uuid")=>"String", Symbol("names")=>"Vector{Vector}", )
...
function OpenAPI.validate_property(::Type{ Node }, name::Symbol, val)
    if name === Symbol("created")
        OpenAPI.validate_param(name, "Node", :format, val, "date-time")
    end
    if name === Symbol("created")
        OpenAPI.validate_param(name, "Node", :pattern, val, r"^\d{4}\-(0[1-9]|1[0-2])\-(0[1-9]|[12][0-9]|3[01])T([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]\.\d{3}Z$")
    end
    if name === Symbol("uuid")
        OpenAPI.validate_param(name, "Node", :format, val, "uuid")
    end
end
